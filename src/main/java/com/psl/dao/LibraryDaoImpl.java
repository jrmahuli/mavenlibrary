...
public class LibraryDaoImpl extends JdbcDaoSupport implements LibraryDao {
	...
	public int bookBorrow(Transaction transaction) {
		String query = "update book set isavailable=false where id="
				+ transaction.getBook_id();
		getJdbcTemplate().update(query);
		query = "insert into transaction(bid,cid,duedate,isreturned) values(?,?,?,0)";
		return getJdbcTemplate().update(query, transaction.getBook_id(),
				transaction.getCustomer_id(), transaction.getDue_date());
	...
	public Date bookReturn(int id) {
		...
		try {
			...
			query = "select * from transaction where isreturned=false and bid="
					+ id;

			transaction = getJdbcTemplate().queryForObject(query,
	...
	@Override
	public Admin validateLogin(String name, String password)
	...
	@Override
	public Admin validateLogin1(String name, String password)
			throws DataNotFoundException {
		try {
			String query = "select * from admin where name=?";
			...
			dataSource
					.setUrl("jdbc:mysql://10.80.34.167:8025/library");

			dataSource.setUsername("root");

			dataSource.setPassword("root");

			// DriverManagerDataSource d= new Datasource
			JdbcTemplate j = new JdbcTemplate();
			j.setDataSource(dataSource);

			return j.queryForObject(query, new RowMapper<Admin>() {
	...
	public boolean addBook1(Book book) {
		try {
			...
			JdbcTemplate j = new JdbcTemplate();
			j.setDataSource(dataSource);
			j.update(query, book.getName(), book.isIsavailable());
			// String sql = "insert into book(name,isavailable) values('" +
			// book.getName() + "', " + book.isIsavailable() + ");";
	...
	@Override
	public boolean registerCustomer1(String name) {
		try {
			String query = "insert into customer(name) values(?)";
	...
	public int getBookId(String book_name) {
		// String
		// query="SELECT id FROM book    where name='"+book_name+"' and isavailable=true limit 1";
